...
public class Pagination {
    ...
    private int ranges;
    private int limit;
    private int firstRangePage; // 구간별 첫페이지
    private int lastRangePage; // 구간별 마지막 페이지
    private int prevRangePage; // 이전 구간 첫페이지 번호
    private int nextRangePage; // 다음 구간 첫페이지 번호
    ...
    public Pagination(int page, int total, int ranges, int limit, HttpServletRequest request){
        ...
        if(lastRangeCnt > rangeCnt ){
            // 마지막 구간이 아닌 경우
            nextRangePage = lastRangePage + 1;
        }
        /*
         * 쿼리스트링 값 유지 처리 - 쿼리스트링 값 중에서 page만 제외하고 다시 조합
         *  예) ?orderStatus=CASH&name=...&page=2 -> page 제외하고 유지
         *      ?page=2 -> ?
         *      없는경우 -> ?
         *
         *      &로 문자열 분리
